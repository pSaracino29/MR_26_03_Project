...
class FrontierDetector(Node):
    def __init__(self):
        ...
        self.frontier_list_publisher = self.create_publisher(PoseArray, 'frontier_list', 10)
    ...
    def detect_frontiers(self):
        ...
        # calcola una sola volta, fuori dal loop, quante celle corrispondono a mezza larghezza del robot
        half_footprint_cells = int(0.7 / self.map.info.resolution)
            ...
            for col in range(1, self.map.info.width-1):
                if grid[row][col] == 0:
                    ...
                    if np.any(neighbors == -1):
                        ...
                        # nuovo filtro, prima di accettare la frontiera
                        window = grid[max(0, row-half_footprint_cells):row+half_footprint_cells+1,
                                    max(0, col-half_footprint_cells):col+half_footprint_cells+1]
                        if np.any(window == 100):
                            continue  # spazio troppo stretto per il footprint del robot,
        ...
        for cluster in clusters:
            ...
            # Cerca la cella reale del cluster più vicina alla media aritmetica
            best_cell = min(cluster, key=lambda c: np.hypot(c[0] - mean_row, c[1] - mean_col))
            x = best_cell[1] * self.map.info.resolution + self.map.info.origin.position.x
            y = best_cell[0] * self.map.info.resolution + self.map.info.origin.position.y
        ...
        self.frontier_list_publisher.publish(pose_array)
